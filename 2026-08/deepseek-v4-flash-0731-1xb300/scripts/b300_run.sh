...
O=/work/out_$TAGP
mkdir -p $O
BASE='"--tensor-parallel-size","1","--gpu-memory-utilization","0.90","--max-model-len","400000","--max-num-batched-tokens","32768","--kv-cache-dtype","fp8","--logprobs-mode","processed_logprobs","--worker-extension-cls","gonka_poc.worker.PoCWorkerExtension","--trust-remote-code"'
...
arm () {
  ...
  while read -r SID BH PK; do
    ...
    timeout 1200 python3 -u /root/collect_artifacts.py --url http://127.0.0.1:8081 --model "$M" \
      ...
      --block-hash "$BH" --public-key "$PK" --gpu 1xB300-SXM6-TP1 --vllm-version "0.25.1-k10-$T" 2>&1 \
...
echo "=== ARM_ON ===";  up "$SPEC" && arm dspark_on
echo "=== ALL_DONE ==="
