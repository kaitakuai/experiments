...
Uses callback-based PoC API:
  /api/v1/inference/pow/init/generate  - start continuous generation
  /api/v1/inference/pow/stop           - stop generation
...
# --- PoC seeds; set from CLI in main() ---
BLOCK_HASH = "artifact_collection_block_v1"
PUBLIC_KEY = "artifact_collection_pk_v1"  # different from bench's 9999
...
def main():
    ...
    parser.add_argument("--block-hash", default="artifact_collection_block_v1",
                        help="PoC block_hash seed (see poc_seeds.json for the standard set)")
    parser.add_argument("--public-key", default="artifact_collection_pk_v1",
                        help="PoC public_key seed (see poc_seeds.json for the standard set)")
    ...
    global BLOCK_HASH, PUBLIC_KEY
    BLOCK_HASH = args.block_hash
    PUBLIC_KEY = args.public_key
    print(f"PoC seeds: block_hash={BLOCK_HASH[:16]}... public_key={PUBLIC_KEY[:16]}...", flush=True)
