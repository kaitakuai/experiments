...
run_mode () {
  ...
  docker exec $C curl -s -X POST http://127.0.0.1:8081/api/v1/inference/up/async \
    ...
    -d "{\"model\":\"$MODEL\",\"dtype\":\"auto\",\"additional_args\":[$EXTRA]}" -m 60 | head -c 200; echo
  ...
  docker exec $C python3 -u /root/collect_artifacts.py --url http://127.0.0.1:8081 \
    --model "$MODEL" --output-dir /root/${MODE}_arts \
    --nonces 1000 --batch-size 32 --logprobs-count 0 \
    --gpu "B300-prod-$TAG" --vllm-version "0.25.1-$MODE" 2>&1 | tee $OUT/${MODE}_collect.log | tail -5
