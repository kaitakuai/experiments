...
LOG=/root/hf/gh-rep-compiled.log; echo "REP_START compiled $(date -u +%H:%M:%S)" > $LOG
...
LAUNCH
  docker exec -d dpoc bash -lc "bash /hf/launch.sh > /hf/gh-rep-compiled-$1.log 2>&1"
...
echo "REP_DONE compiled $(date -u +%H:%M:%S)" >> $LOG
