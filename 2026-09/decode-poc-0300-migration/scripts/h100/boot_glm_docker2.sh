#!/usr/bin/env bash
# boot_glm_docker.sh + two hooks: CCJSON (whole --compilation-config JSON) and PRE (shell run inside the container before serve).
set -u
NAME=${NAME:-glm030}; OUT=${OUT:-/root/rout/glm}; N=${N:-256}; CAPTURE=${CAPTURE:-$N}; GMU=${GMU:-0.95}; MNBT=${MNBT:-8192}; KVDT=${KVDT:-fp8}
CCJSON=${CCJSON:-"{\"max_cudagraph_capture_size\":$CAPTURE}"}
...
[ -n "${PRE:-}" ] && docker exec "$NAME" bash -lc "$PRE"
docker exec -d "$NAME" bash -lc "vllm serve /models/GLM-5.3-Flash --served-model-name zai-org/GLM-5.3-Flash --host 127.0.0.1 --port 8000 \
  ...
  --compilation-config '$CCJSON' ${EXTRA:-} > /rout/$(basename $OUT)/server.log 2>&1"
