...
MLNODE_URL = os.environ.get("MLNODE_URL", "http://127.0.0.1:8081")
...
MODEL_NAME = os.environ.get("MODEL", "deepseek-ai/DeepSeek-V4-Flash")
...
POW_CONFIG = {
    "block_hash": os.environ.get("POC_BLOCK_HASH", "TEST_BLOCK"),
    "block_height": 100,
    "public_key": os.environ.get("POC_PUBLIC_KEY", "test_pub_keys"),
    "node_id": 0,
    "node_count": 1,
    "params": {
        "model": MODEL_NAME,
...
def start_vllm_if_needed():
    return True  # vLLM started externally with run-specific args
    if is_vllm_running():
...
def send_validation_request(artifacts: List[dict]) -> dict:
    ...
    response = requests.post(
        f"{MLNODE_URL}{API_PREFIX}/pow/generate",
...
def run_phase1_generation_validation():
    ...
    elapsed_time = (t_before + t_after) / 2 - start_time
    stop_generation()
...
def run_phase3_autobatch_sizing():
    ...
    for batch_size in BATCH_SIZES_TO_TEST:
        ...
        # Снимок на границе окна: сначала читаем счётчик, ПОТОМ останавливаем.
        # Интервал берём серединой round-trip'а /stats — нонсы, доставленные во
        # время самого запроса, иначе считались бы без своего времени.
        t_before = time.monotonic()
        ...
        # Добор: колбэки, прилетевшие после границы, только показываем.
        # В пропускную способность они НЕ входят — их время в интервал не попало.
        time.sleep(1.0)
        post_boundary = get_batch_receiver_stats()['total_nonces'] - boundary_stats['total_nonces']
        ...
        if post_boundary:
            print(f"  Исключено после границы: {post_boundary} нонсов")
