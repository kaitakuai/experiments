# Kimi-K2.6 INT4 — 4×B200 — q-int4-k2 image validation

**Date:** 2026-05-21
**Model:** `moonshotai/Kimi-K2.6` (compressed-tensors INT4 packed, MLA, 384 routed × top_k=8)
...
Validation of the **dedicated B200 INT4 image** `mlnode-b200-kimi-k2-6:0.2.13-vllm0.20.0-q.int4-k2`.
Unlike the generic `b200-k5-kimi-1` image (which ships eager mode and whose
weights are not INT4-packed, leaving `VLLM_USE_FLASHINFER_MOE_INT4` inert),
this image ships **genuine compressed-tensors INT4-packed weights** (config
reports `quant_method: compressed-tensors` + `pack`) and runs with
**CUDA graphs** (`cudagraph_mode=FULL_AND_PIECEWISE`), expert-parallel on,
at `gpu-memory-utilization=0.93`.
...
0.187–0.189 cross-anything INT4 floor).
...
Quant         : compressed-tensors, INT4 packed  ("quant_method":"compressed-tensors", "pack")
...
--trust-remote-code
ENV VLLM_USE_FLASHINFER_MOE_INT4=1      # active here (weights ARE int4-packed), sm_100 only
ENV VLLM_USE_FLASHINFER_MOE_FP4=1  VLLM_USE_FLASHINFER_MOE_FP8=1
```
...
| Step | Result |
|---|---:|
| Model load to VRAM (TP=4, INT4) | 141.37 GiB / GPU, 30.3 s |
...
result (also 2240 @ batch=32) — i.e. the INT4-packed weights + CUDA graphs do
not change PoC throughput vs the k5 eager build (PoC runs a separate
forward-only path, not the inference decode/CUDA-graph path).
...
|---|---:|---:|---|
| **q-int4-k2 (050) ↔ b200-k5 eager (051)** | **0.1874** | 5 (2.5%) | **PASS** p=0.37 |
| **q-int4-k2 (050) ↔ b300 eager (001)** | **0.1888** | 3 (1.5%) | **PASS** p=0.77 |
| **q-int4-k2 (050) ↔ b300 CG (002)** | **0.1879** | 3 (1.5%) | **PASS** p=0.77 |

All mean L2 in the **0.187–0.189** corridor — the fundamental Kimi-K2.6 INT4
...
`0.02/0.001` self-validation defaults FAIL across all cross-anything INT4 pairs
(expected).

This confirms the q-int4-k2 image produces honest, cross-validatable nonces
...
- [`artifacts/l2_4way.json`](artifacts/l2_4way.json) — canonical pairwise L2 +
  binomtest results across all 4 live network nodes (050 q-int4-k2, 051 k5,
  001/002 b300)
...
- Generic B200 k5 image (eager, non-int4-packed weights):
...
- FlashInfer MXINT4 MoE provenance: vLLM PR
  [#32437](https://github.com/vllm-project/vllm/pull/32437)
  (env `VLLM_USE_FLASHINFER_MOE_INT4=1`, sm_100 only)
